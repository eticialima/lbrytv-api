...
def get_claim_id():
    ...
    response = requests.post(HOST, json={"method": "resolve", "params": {
                             "urls": "lbry://@JouninReact#d"}}).json()
...
def get_streams(claim_id, limit=None):
    ...
    response = requests.post(HOST, json={"method": "claim_search", "params": {
                             "channel_ids": [claim_id], "order_by": "release_time", }}).json()
    ...
    for page in range(1, pages+1):
        ...
        response = requests.post(HOST, json={"method": "claim_search", "params": {
                                 "page": page, "channel_ids": [claim_id], "order_by": "release_time"}}).json()

        urls += [item["canonical_url"] for item in response["result"]
                 ["items"] if item["value_type"] == "stream"]

        sd_hashes += [item["value"]["source"]["sd_hash"]
                      for item in response["result"]["items"] if item["value_type"] == "stream"]
