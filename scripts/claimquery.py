...
def get_video_in_playlist(claim_id_list): 
    query = "select * from claim where claim_id in '{}'+ORDER+BY+release_time+DESC+LIMIT+100".format(claim_id_list).replace(" ", "+")    
...
def VideoDetalhes(request, claim_id): 
    ...
    query = "select * from claim where claim_id = '{}'+and+type='claimlist'+ORDER+BY+release_time+DESC+LIMIT+100".format(claim_id).replace(" ", "+")
